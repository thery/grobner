...
(*                                                                            *)
(*  mino x y      : the minimum of two elements in a cpoType                  *)
(*  mlcm m m'     : the least common multipler of two monomials               *)
(*  mlast p       : the last monomial of a polynomial for the order <%O       *)
(*  plt p p'      : p is lexicographically smaller than p' for the orderr <%O *)
...
(*  irreducible L p : p is irreducible by a polynomial in p                   *)
(*  mreducef L p  : return an option type that gives a witness of a reduction *)
(*                  of p by a polynomial in L                                 *)
(*  mreduceplus L p q : p can be reduced in multiple step to q                *)
(*  mreducestar L p q : p can be reduced in multiple step to q and q is       *)
(*                      irreducible                                           *)
...
(*  splt L L'     : the sequence of polynomial L' is p :: L where p cannot    *)
(*                  be reduced by L                                           *)
(*  mbuch_all     : complete the sequence L to build a Grobner basis using    *)
...
(******************************************************************************)
(*          Complement to the multinomial library                             *)
(******************************************************************************)

(* Minimum in a cpo *)
Definition mino {T : cpoType} (x: T) y := if (x < y)%O then x else y.

Lemma minoo {T: cpoType} : idempotent (@mino T).
Proof. by move=> x; rewrite /mino; case: (_ < _)%O. Qed.

Lemma leo_minl {T: cpoType} (x y : T) :  (@total T <=%O) -> (mino x y <= x)%O.
Proof.
move=> HT; rewrite /mino; case E: (_ < _)%O; first by rewrite leoo.
by rewrite letNgt // E.
Qed.

Lemma leo_minr {T: cpoType} (x y : T) : (mino x y <= y)%O.
Proof.
by rewrite /mino; case E: (_ < _)%O; [rewrite ltoW ?E | rewrite leoo].
Qed.

Lemma minoCA {T : cpoType} :  (@total T <=%O) -> left_commutative (@mino T).
Proof.
move=> HT x y z; rewrite /mino.
case E: (y < z)%O; case E1: (x < z)%O; rewrite ?E //.
- rewrite (lttNge HT) leo_eqVlt.
  have[/eqP<-/=|_]/= := boolP (_ == _); first by rewrite if_same.
  by case: (_ < _)%O.
- by rewrite (lttNge HT) (leo_trans (_ : (_ <= z))%O) //; 
       [rewrite ltoW ?E | rewrite (letNgt HT) E1].
by rewrite (lttNge HT) (leo_trans (_ : _ <= z)%O) //;
     [rewrite ltoW ?E | rewrite (letNgt HT) E].
Qed.

Lemma lem_leo m1 m2 : (m1 <= m2)%MM -> (m1 <= m2)%O.
Proof.
by move=> H; rewrite -(submK H) -{1}[m1]mpoly.add0m lem_add // le0o.
Qed.
...
Lemma ltom_wf : well_founded (@lt (@multinom_posetType n)).
...
elim: l a => //= [a|a l IH a1].
  by by rewrite big_nil minoo inE eqxx.
...
rewrite big_cons !inE => /orP[/eqP<-|].
  by apply: leo_minl lem_total.
elim: l => //= m2 l IH; rewrite inE big_cons=> /orP[/eqP<-|/IH H].
  by apply: leo_trans (leo_minr _ _) (leo_minl _ _ _); exact: lem_total.
rewrite minoCA; last exact: lem_total.
by apply: leo_trans (leo_minr _ _) H.
Qed.
...
apply: (iffP hasP)=> [[m Im /and3P[NIm /allP /=Hq /allP Hp]]|[m [Im NIm HA]]].
  ...
  have := Hq m1; have := Hp m1; do 2 case: (_ \in _) => //=.
    by rewrite (letNgt (lem_total)) Lm => /(_ is_true_true).
  by rewrite (lttNge lem_total) [(m <= _)%O]ltoW // => _  /(_ is_true_true).
exists m => //; apply/and3P; split=>//; apply/allP=> m1 Im1.
  have [] := boolP (_ < m)%O; rewrite // -(letNgt lem_total) //.
  by rewrite leo_eqVlt => /orP[/eqP<-|/HA<-].
by have [] := boolP (_ <= m)%O; rewrite // -(lttNge lem_total) =>/HA->.
Qed.

Lemma plt_mlead p q : (mlead p < mlead q)%O -> (p < q).
Proof.
have [/eqP->|Zq] := boolP (q == 0); first by rewrite mlead0 lto0.
...
have /orP[Lm|Lm] := lem_total m1 m2.
  apply/pltP; exists m2; split=> [||m3 Lm3] //.
    by move: Lm; rewrite leo_eqVlt => /orP[/eqP<-//|/HAm1->].
  by rewrite -HAm2 // HAm1 // (leo_lto_trans Lm).
apply/pltP; exists m1; split=> [||m3 Lm3] //.
  by move: Lm; rewrite leo_eqVlt => /orP[/eqP<-//|/HAm2<-].
by rewrite HAm1 // -HAm2 // (leo_lto_trans Lm).
Qed.
...
by rewrite !mcoeff_msupp !mcoeff_gt_mlead // (lto_trans _ Lm).
Qed.
...
have [/eqP Eq|Dq] := boolP (mlead q == m).
  left; have [/Lm HH|] := boolP (m < mlead p)%O.
    ...
    rewrite leo_eqVlt=> /orP[] // /eqP Ep.
    by case/negP: NIm; rewrite -Eq -Ep mlead_supp.
  rewrite -(letNgt lem_total) leo_eqVlt Eq => /orP[/eqP Ep|] //.
...
have: (mlead q <= mlead p)%O.
  ...
  by rewrite lto_neqAle eq_sym Dq msupp_le_mlead.
rewrite leo_eqVlt => /orP[/eqP->|] // /plt_mlead /(plt_trans Lp).
by rewrite plt_anti.
Qed.
...
have [/eqP Eq|Dq] := boolP (mlast q == m); last first.
  ...
  have: (mlast q < m3)%O.
    by apply: leo_lto_trans Im3; apply: mlast_leo.
  by rewrite lto_neqAle [_ == m3]eq_sym => /andP[->].
...
- rewrite [p]mpolyE (bigID (fun i => (m < i)%O)) /=; congr (_ + _).
  ...
  apply: eq_bigl=> // m1.
  rewrite -(letNgt lem_total) leo_eqVlt.
...
- apply: uniq_perm_eq=> [||m1]; first by apply: msupp_uniq.
  ...
  - apply/flattenP/andP=>[[m2 /mapP[m3]]|[Dm LL]].
      ...
      have: (mlast q < m3)%O by rewrite (leo_lto_trans (mlast_leo _) H1).
      by rewrite eq_sym lto_neqAle; case/andP.
    exists [::m1]; last by rewrite inE.
    apply/mapP; exists m1.
      rewrite mem_filter ?LL.
      suff F : (m < m1)%O by rewrite F Lm.
      by have := mlast_leo LL; rewrite leo_eqVlt eq_sym (negPf Dm) Eq.
    by rewrite msuppMCX // -mcoeff_msupp Lm // lto_neqAle -Eq eq_sym Dm mlast_leo.
  - by apply: msupp_uniq.
...
have : (m2 < m1)%O.
by rewrite (lto_trans H1) // -Eq.
by  rewrite Em1 ltoo.
move/negPf->.
by move: Lm1; rewrite eq_sym lto_neqAle => /andP[/negPf->].
Qed.
...
have F1 : mlast q2 \notin msupp p1.
  apply/negP=> HH; 
  suff: (mlast q < mlast q)%O by rewrite ltoo.
  by apply: lto_trans (Lp1 _ _) (Lp2 _ F0).
...
have F3 : mlast (p1 + q2) = mlast q2.
  apply: mlastE=> [|m1 /msuppD_le].
    ...
    suff: (m1 < m1)%O by rewrite ltoo.
    by apply: lto_trans O1.
  rewrite mem_cat=> /orP[/Lp1 O1|/mlast_leo//].
  by apply: ltoW; apply: lto_trans O1; rewrite -(eqP Em).
have F4 : (p1 + q2)@_(mlast q2) = q2@_(mlast q2).
  ...
  by apply: lto_trans F2 _; rewrite (eqP Em) (Lp1 _ (mlast_supp _)).
apply: Acc_intro => q3.
...
have [/Lp2|] := boolP (m1 \in msupp q2); first by rewrite (lttNge lem_total) ltoW.
by move: Lm1; rewrite andbF lto_neqAle eq_sym => /andP[/negPf->].
Qed.
...
have [/eqP->|ZX] := boolP ('X_[(m - mlead q)] == 0 :> {mpoly R[n]}).
  by rewrite mulr0 mlead0 (leo_lto_trans (le0o _) Lm).
have [/eqP->|Zq] := boolP (q == 0).
  by rewrite mul0r mlead0 (leo_lto_trans (le0o _) Lm).
...
have [/eqP->|Zlp] := boolP (p@_m / mleadc q == 0).
  by rewrite scale0r mul0r mlead0 maxo0 leo_refl.
rewrite geo_max; last by exact: lem_total.
rewrite leo_refl -scalerAl mleadZ //.
apply: leo_trans (mleadM_le _ _) _.
...
- by apply: mreduce_lt.
rewrite lttNge; last by exact: lem_total.
apply/negP=> HH.
have: p < p.
  apply: plt_trans (mreduce_lt Rp).
  apply: plt_mlead.
by apply: lto_leo_trans HH.
by rewrite plt_anti.
Qed.
...
move: Lm1; rewrite lto_neqAle => /andP[/negPf-> _].
...
wlog: m1 m2 r1 r2 Im1 Zr1 Lr1 Ir1 Im2 Zr2 Lr2 Ir2 / (m2 <= m1)%O => [HW|Lm2].
  ...
  case: (HW m2 m1 r2 r1 Im2 Zr2 Lr2 Ir2 Im1 Zr1 Lr1 Ir1) => //.
    rewrite letNgt ?lto_neqAle ?negb_and ?Lo 1?orbC //.
    by exact: lem_total.
...
- pose t : {mpoly R[n]} := mleadc p *: 'X_[mlead p]. 
  ...
  apply/mreduceP; exists m1; exists r1; split=> //.
    ...
    rewrite mleadM //.
      ...
      by rewrite lto_neqAle Em1 Em2 -Em1.
...
have F3 : (q1 < p).
  ...
  rewrite !mcoeff_msupp mcoeffB mcoeffZ mcoeffX.
  move: Lm3; rewrite lto_neqAle => /andP[/negPf-> _].
  by rewrite mulr0 subr0.
...
- have->: mdiv m1 p r1 = t + q2.
  ...
  apply: mreduceplus_compatX => //.
    apply: leo_lto_trans (mreduce_lead F1) _.
...
- have->: mdiv m2 p r2 = t + q3.
  ...
  apply: mreduceplus_trans (mreduceplusfE _).
  apply: mreduceplus_compatX => //.
    apply: leo_lto_trans (mreduce_lead F2) _.
    by apply: ltm_mleadD => //; apply: mreduce_neq0 F1.
